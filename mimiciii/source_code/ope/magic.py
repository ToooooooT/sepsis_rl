...
# put below code in main to evaluate

''' magic evaluation
magic = MAGIC(gamma=agent.gamma)
done_indexs = np.where(test_data['done'] == 1)[0]
start_indexs = np.append(0, done_indexs[:-1] + 1)
policy_actions_list = []
rewards_list = []
base_propensity_list = []
target_propensity_list = []
qhat_list = []
for i in range(done_indexs.shape[0]):
    policy_actions_list.append(policy_actions[start_indexs[i]:done_indexs[i] + 1].reshape(-1,).tolist())
    rewards_list.append(test_data['r'][start_indexs[i]:done_indexs[i] + 1].reshape(-1,).tolist())
    tmp = []
    for act in test_data['a'][start_indexs[i]:done_indexs[i] + 1].reshape(-1).astype(int).tolist():
        x = np.full(25, 0.001)
        x[act] = 1 - 0.001 * 24
        tmp.append(x)
    base_propensity_list.append(tmp)
    target_propensity_list.append([policy_action_probs[i, :] for i in range(start_indexs[i], done_indexs[i] + 1)])
    states = torch.tensor(test_data['s'], dtype=torch.float, device=agent.device)
    with torch.no_grad():
        q_values = fqe.Q(states).cpu().numpy()
    qhat_list.append([q_values[i, :] for i in range(start_indexs[i], done_indexs[i] + 1)])
info = [policy_actions_list,
        rewards_list,
        base_propensity_list,
        target_propensity_list,
        qhat_list
        ]
ret = magic.evaluate(info, num_j_steps=1, is_wdr=True)
print(ret[0])
print(ret[1])
print(ret[2])
print(ret[3])
