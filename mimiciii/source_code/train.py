...
def training(model, valid, config, valid_dataset, id_index_map):
    ...
    for i in range(1, config.EPISODE + 1):
        ...
        if i % 1000 == 0:
            ...
            model.save()
            model.save_td(loss / 1000, i // 1000)
...
def plot_training_loss(model):
    ...
    with open(os.path.join(model.log_dir, 'td.csv'), newline='') as f:
        reader = csv.reader(f)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--use_pri", type=bool, help="use priority replay", dest="use_pri", default=True)
    ...
    path = os.path.join('./log', f'batch_size-{config.BATCH_SIZE} episode-{config.EPISODE} use_pri-{config.USE_PRIORITY_REPLAY}')
